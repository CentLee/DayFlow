...
The supervisor adds no model invocation or token pool. Every dispatched issue inherits a preemptive phase execution slice: 120 seconds for primary work and 90 seconds each for review and remediation. The runner terminates a slice breach before a later model stage can begin. It also records a 400,000 aggregate **billable** token ceiling and cumulative phase ceilings of 220,000 for primary work, 100,000 for review, and 180,000 for remediation. Codex can emit usage only at completion, so these token ceilings are fail-closed admission and diagnostic controls rather than real-time spend guarantees. A completed execution that reports its overage only after exit retains its validated output and is deterministically published; no later model stage can start under the exhausted accounting state. Prompt and output caps and a five-minute no-progress limit remain unchanged. Billable tokens are uncached input plus output; cached input remains separately persisted as raw observability and never consumes a token cap. Review follow-up and requested changes start a fresh bounded session containing only the active findings; they never resume an unbounded prior transcript. Codex runs with `--ignore-user-config`, preventing unrelated local MCP configuration from entering the task. A limit breach preserves the worktree and blocks the issue.

The Primary Agent must leave a pushed branch, at least one commit beyond `origin/<declared-base>`, an open PR targeting that base, and non-empty proof sections. The runner creates publication commits with the explicit `DAYFLOW_GIT_AUTHOR_NAME` and `DAYFLOW_GIT_AUTHOR_EMAIL` identity (defaulting to `DayFlow Runner` and its no-reply address), so clean runners do not depend on global Git configuration. It then invokes `review-agent` with `gpt-5.6-terra/high`. P0-P2 findings are posted to the PR and returned once to the same Primary Agent owner in a fresh bounded session. A second blocking review result blocks the issue.
...
- Requested changes: run `reconcile`, then `run CEN-N`; the same Primary Agent owner receives the current findings in a fresh bounded session.

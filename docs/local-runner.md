...
The supervisor adds no model invocation or token pool. Every dispatched issue inherits one hard aggregate issue budget and one observation threshold: `DAYFLOW_RESOURCE_TOKEN_LIMIT` defaults to 120,000 uncached-input-plus-output tokens, while `DAYFLOW_CONTEXT_TOKEN_LIMIT` defaults to a 500,000-token total-context warning threshold including cached input. `DAYFLOW_TOKEN_LIMIT` remains a compatibility alias for the resource limit when the new variable is unset. Cached-context reporting therefore remains visible without overriding the authoritative uncached-input-plus-output resource bound.

Before every primary, resume, or review launch, the runner enforces the 32 KiB `DAYFLOW_PROMPT_LIMIT_BYTES` cap. During execution it bounds captured stdout/stderr with the 5 MiB `DAYFLOW_LOG_LIMIT_BYTES` cap, total wall time at 20 minutes, and no-progress time at five minutes. Final usage emitted as the process exits is accounted before classification. Every invocation contributes one cumulative usage snapshot rather than summing repeated JSONL snapshots. Local issue state records cached input, uncached input, output, total tokens, and invocation count under `.usage`; `.token_budget` records both formulas, configured values, and current use. A total-context threshold crossing persists `.context_observation` with the usage, threshold, first/latest phase, timestamps, count, and operator-facing reason. The runner emits that warning again before a later invocation or same-session resume, so context-over-threshold reuse is never silent, but a successful invocation under the resource limit stays successful. Resource, prompt, command-output, stall, and timeout guards still terminate the Codex process tree, preserve the worktree, and block with their specific reasons; model rejection and other nonzero exits are unchanged.
...
- Model, resource-token, timeout, or delivery failure: inspect `status` and `.dayflow/logs`, retain the worktree, resolve the cause, then deliberately return the issue to a runnable state. A total-context observation needs operator review but does not itself require blocked-state recovery.

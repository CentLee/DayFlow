...
The supervisor adds no model invocation or token pool. Every dispatched issue inherits the runner's resource boundaries: a 400,000 aggregate **billable** token ceiling, plus cumulative phase ceilings of 220,000 for primary work, 100,000 for review, and 180,000 for remediation. The aggregate cap is absolute even when phase ceilings leave room; phase accounting prevents one lifecycle stage from spending the complete issue budget. Prompt and output caps, a 20-minute invocation limit, and five-minute no-progress limit remain unchanged. Billable tokens are uncached input plus output; cached input remains separately persisted as raw observability and never consumes a token cap. Admission, live monitoring, persisted aggregate and phase accounting, and post-exit checks use that same billable value. A limit breach preserves the worktree and blocks the issue.

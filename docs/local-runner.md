...
The supervisor adds no model invocation or token pool. Every dispatched issue inherits two aggregate issue token budgets: `DAYFLOW_RESOURCE_TOKEN_LIMIT` defaults to 120,000 uncached-input-plus-output tokens, while `DAYFLOW_CONTEXT_TOKEN_LIMIT` defaults to 500,000 total tokens including cached input. `DAYFLOW_TOKEN_LIMIT` remains a compatibility alias for the resource limit when the new variable is unset. This keeps bounded high-reasoning work viable while stopping a cache-heavy session well before multi-million-token context use.

Before every primary, resume, or review launch, the runner enforces the 32 KiB `DAYFLOW_PROMPT_LIMIT_BYTES` cap. During execution it bounds captured stdout/stderr with the 5 MiB `DAYFLOW_LOG_LIMIT_BYTES` cap, total wall time at 20 minutes, and no-progress time at five minutes. Final usage emitted as the process exits is accounted before success, so a late report can still trip either token guard. Every invocation contributes one cumulative usage snapshot rather than summing repeated JSONL snapshots. Local issue state records cached input, uncached input, output, total tokens, and invocation count under `.usage`; `.token_budget` records both formulas, configured limits, and current use. A runner guard terminates the Codex process tree, preserves the worktree, and blocks with its specific prompt, command-output, token, stall, or timeout reason rather than a model-rejection reason.
